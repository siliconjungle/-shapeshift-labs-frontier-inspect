...
Cross-package inspection primitives for Frontier apps. `frontier-inspect` turns registry graphs, Playwright samples, route impacts, tests, migrations, event logs, and telemetry-like records into one AI-readable evidence bundle.

It is intentionally DOM-neutral and Playwright-neutral: adapters pass structural objects in, and this package keeps the shared query, impact, redaction, JSONL, feature-map, and report logic in one small runtime package.
...
```sh
npm install @shapeshift-labs/frontier-inspect
```
...
import {
  ...
  queryInspectBundle,
  traceInspectImpact
} from '@shapeshift-labs/frontier-inspect';

const routeArtifact = createInspectArtifactFromRouteManifest({
  routes: [
    {
      id: 'todos.detail',
      pattern: 'route:/todos/:id',
      resource: 'route:/todos/:id',
      feature: 'todos',
      reads: ['entities.todos'],
      writes: ['ui.route']
    }
  ]
}, { id: 'todos.routes', feature: 'todos' });
...
const routeImpact = createInspectArtifactFromRouteImpact({
  ...
  features: ['todos']
}, { id: 'todos.route-impact' });

const migrationArtifact = createInspectArtifactFromMigrationReport({
  registryId: 'todos',
  steps: [{ id: 'todos.v2', reads: ['entities.todos'], writes: ['entities.todos'] }],
  warnings: [{ code: 'todos.missing-name', severity: 'warning', path: 'entities.todos.1.name' }]
}, { id: 'todos.migration', feature: 'todos' });

const telemetryArtifact = createInspectArtifactFromLogRecords([
  {
    id: 'log:todo-save',
    type: 'action',
    level: 'info',
...
const eventLogArtifact = createInspectArtifactFromEventLog([
  {
    offset: 1,
    key: 'todo.save',
    headers: { feature: 'todos', resource: 'entity:todo' },
    value: { kind: 'frontier.patch' }
  }
]);

const benchmarkArtifact = createInspectArtifactFromBenchmarkReport({
  package: '@app/todos',
...
const touched = traceInspectImpact(bundle, { paths: ['entities.todos'] });
const query = queryInspectBundle(bundle, { features: ['todos'] });
...
- Artifact: a structural source of evidence, such as Playwright timeline, route impact, migration report, benchmark, test output, registry graph, or event log.
...
The package includes structural adapters for the package family surfaces that need to become inspectable without becoming runtime dependencies:

- Route: route manifests and route impact reports become route resources, registry entries, and queryable artifacts.
...
The benchmark writes `benchmarks/results/frontier-inspect-package-bench-latest.json` when run in the monorepo. These are Frontier-only package measurements. It measures bundle creation, query, impact, feature-map, JSONL encode/decode, redaction, proof generation, and report creation using this package's own fixtures only.

## Boundary

`frontier-inspect` depends only on `@shapeshift-labs/frontier`. It does not import DOM, Playwright, route, logging, migration, CRDT, state, scheduler, or framework packages. Those packages should pass structural evidence into this package rather than becoming runtime dependencies.

Bundle queries use a lazy `WeakMap` index for exact feature, package, artifact kind, tag, file, path, resource, entry, record, status, severity, and event-id filters. The bundle itself stays plain JSON-like data, so evidence can still be exported, replayed, redacted, and consumed by AI tooling without a live app runtime.
